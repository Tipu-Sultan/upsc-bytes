...
const schema = z.object({ title: z.string().min(2).max(180), categoryId: z.string().min(1), imageUrl: z.string().url(), cloudinaryPublicId: z.string().min(1), description: z.string().max(1000).optional(), aiSummary: z.string().max(700).optional(), examAngle: z.string().max(700).optional(), keyPoints: z.array(z.string().max(250)).max(5).optional(), tags: z.array(z.string().max(50)).max(20).optional(), status: z.enum(['draft', 'published']).optional() });
