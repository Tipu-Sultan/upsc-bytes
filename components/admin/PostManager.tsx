...
const emptyForm = (categoryId = ''): FormState => ({ id: undefined, title: '', categoryId, description: '', tags: '', imageUrl: '', publicId: '', status: 'published' });
...
export function PostManager({ categories, initial }: { categories: CategoryDTO[]; initial: PostDTO[] }) {
  ...
  function edit(post: PostDTO) {
    setForm({ id: post.id, title: post.title, categoryId: post.categoryId, description: post.description ?? '', tags: post.tags.join(', '), imageUrl: post.imageUrl, publicId: post.cloudinaryPublicId, status: post.status });
  ...
  async function aiAssist() {
    ...
    try {
      ...
      patch({ description: data.data.summary, tags: data.data.tags.join(', ') });
